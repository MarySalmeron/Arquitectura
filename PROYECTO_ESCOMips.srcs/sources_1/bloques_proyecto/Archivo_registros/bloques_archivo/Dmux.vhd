library IEEE;
use IEEE.STD_LOGIC_1164.ALL;

entity Dmux is
    Port ( WR : in STD_LOGIC;
           WriteReg : in STD_LOGIC_VECTOR (3 downto 0);
           l : out STD_LOGIC_VECTOR (15 downto 0));
end Dmux;

architecture Behavioral of Dmux is
signal aux_l : STD_LOGIC_VECTOR (15 downto 0);
begin   

aux_l <= "0000000000000001" when (WriteReg = "0000") else
          "0000000000000010" when (WriteReg = "0001") else
          "0000000000000100" when (WriteReg = "0010") else
          "0000000000001000" when (WriteReg = "0011") else
          "0000000000010000" when (WriteReg = "0100") else
          "0000000000100000" when (WriteReg = "0101") else
          "0000000001000000" when (WriteReg = "0110") else
          "0000000010000000" when (WriteReg = "0111") else
          "0000000100000000" when (WriteReg = "1000") else
          "0000001000000000" when (WriteReg = "1001") else
          "0000010000000000" when (WriteReg = "1010") else
          "0000100000000000" when (WriteReg = "1011") else
          "0001000000000000" when (WriteReg = "1100") else
          "0010000000000000" when (WriteReg = "1101") else
          "0110000000000000" when (WriteReg = "1110") else
          "1000000000000000";

with WR select   
    l <= (others=>'0') when '0',
            aux_l when others;

--process(WriteReg)
--begin
--    if(WR='0') then
--        l <= (others=>'0');
--    else
--        case WriteReg is
--            when "0000" => l <= "0000000000000001";
--            when "0001" => l <= "0000000000000010";
--            when "0010" => l <= "0000000000000100";
--            when "0011" => l <= "0000000000001000";
--            when "0100" => l <= "0000000000010000";
--            when "0101" => l <= "0000000000100000";
--            when "0110" => l <= "0000000001000000";
--            when "0111" => l <= "0000000010000000";
--            when "1000" => l <= "0000000100000000";
--            when "1001" => l <= "0000001000000000";
--            when "1010" => l <= "0000010000000000";
--            when "1011" => l <= "0000100000000000";
--            when "1100" => l <= "0001000000000000";
--            when "1101" => l <= "0010000000000000";
--            when "1110" => l <= "0100000000000000";
--            when others => l <= "1000000000000000";
--        end case;
--    end if;
--end process;

end Behavioral;
